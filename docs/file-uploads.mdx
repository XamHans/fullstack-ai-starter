    ...
    <div className="space-y-4">
      ...
      {uploadedFile && (
        <div>
          ...
          <img src={uploadedFile} alt="Profile" className="w-32 h-32 rounded-full" />
        </div>
      )}
    </div>
  );
...
Visit `/playground/file-upload` to test all upload features interactively with different file types and configurations.